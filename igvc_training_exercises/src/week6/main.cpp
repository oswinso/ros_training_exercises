...
void oswinPoseCallback(const geometry_msgs::PoseStamped& pose)
{
  ...
  std::vector<double> costs(num_primitives, 0.0);
  std::vector<nav_msgs::Path> paths(num_primitives);
  ...
  {
    double angular_velocity = angles_start + i * angle_increment;
  ...
  double best_angular_velocity = angles_start + best_index * angle_increment;
  geometry_msgs::Twist twist_message;
  twist_message.linear.x = linear_velocity;
  twist_message.angular.z = best_angular_velocity;
